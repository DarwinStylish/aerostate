...
def test_step_basic_motion_applies_gravity_without_thrust() -> None:
    ...
    assert next_state.altitude_m == pytest.approx(100.0 - GRAVITY_MPS2)


def test_step_basic_motion_applies_forward_thrust() -> None:
    next_state = step_basic_motion(
    ...
    assert next_state.forward_position_m == 14.0
